...
export async function POST(
  ...
  try {
    ...
    // 이미 확정된 경우 → 기존 room_id 반환 (멱등)
    if (post.chat_room_id) {
      return NextResponse.json({ chat_room_id: post.chat_room_id });
    }
    ...
    if (roomError) {
      // 23505 = unique violation → 이미 다른 요청이 생성함 → 기존 room 반환
      if (roomError.code === '23505') {
        const { data: existing } = await admin
          .from('gather_chat_room')
          .select('id')
          ...
          .maybeSingle();
        if (existing) {
          await admin.from('gather_post').update({ chat_room_id: existing.id }).eq('id', postId);
          return NextResponse.json({ chat_room_id: existing.id });
        }
      }
      console.error('[gather confirm] room create error:', roomError);
      return NextResponse.json({ error: 'Failed to create chat room' }, { status: 500 });
    ...
    const memberUserIds = participants.map((p: any) => p.user_id);
    if (!memberUserIds.includes(user.id)) memberUserIds.push(user.id);
    const members = memberUserIds.map((uid: string) => ({ room_id: room.id, user_id: uid }));
    ...
    // gather_post에 chat_room_id 업데이트
    await admin
      .from('gather_post')
      .update({ chat_room_id: room.id })
      .eq('id', postId);

    return NextResponse.json({ chat_room_id: room.id });
  } catch (err) {
    console.error('[gather confirm] error:', err);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
